    ...
    results = [
        {
            ...
            "publisher": extract_first(
                result.xpath('.//span[contains(@class,"mr-5 cite-co")]/text()')
            ),
            "publish_date": publish_time(
                extract_first(result.xpath('.//span[contains(@class,"s-time")]/text()'))
            ),
...
def get_search_url(query: str, latest: bool = True, country: str = "us") -> str:
    url_country = (
        (country.lower() + ".") if country.lower() != "us" and len(country) > 0 else ""
    )
    # &fr=uh3_news_vert_gs'
