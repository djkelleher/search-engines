...
def extract_search_results(
    ...
    results = [
        {
            "url": extract_first(
                result.xpath(
                    './/a[@class="PartialSearchResults-item-title-link result-link"]/@href'
                )
            ),
            "title": extract_first(
                result.xpath(
                    './/a[@class="PartialSearchResults-item-title-link result-link"]/text()'
                )
            ),
            "preview_text": extract_first(
                result.xpath('.//p[@class="PartialSearchResults-item-abstract"]/text()')
            ),
    ...
    next_page_url = extract_first(
        root.xpath('//li[@class="PartialWebPagination-next"]/a/@href')
    )
    if next_page_url:
