...
async def parse_page(resp: ClientResponse,
    ...
    results = [{
        ...
        'title':
        join_all(result.xpath("./a//div[@role='heading']/text()")),
        'preview_text':
        join_all(
            result.xpath(
                ".//div[@class='dbsr']/a//div[@class='eYN3rb']/text()")),
        'publisher':
        extract_first(result.xpath('./a//div[@class="pDavDe RGRr8e"]/text()')),
        'publish_date':
        get_publish_time(
            extract_first(
                result.xpath('.//span[@class="eNg7of"]/span/text()'))),
        'page_url':
        page_url,
        'page_num':
        page_num,
