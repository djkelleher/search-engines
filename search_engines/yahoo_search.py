            ...
            results.append(
                {
                    "url": url,
                    "title": join_all(
                        result.xpath(".//h3[contains(@class,'title')]//a//text()")
                    ),
                    "preview_text": join_all(
                        result.xpath(".//div[@class='compText aAbs']//text()")
                    )
                    or join_all(result.xpath(".//span[@class=' fc-falcon']//text()")),
...
def get_search_url(query: str, latest: bool = True, country: str = "us") -> str:
    url_country = f"{country.lower()}." if country and country.lower() != "us" else ""
