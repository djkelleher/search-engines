...
def extract_search_results(
    ...
    page_number = extract_first(
        root.xpath('//span[@class="pagination__num pagination__num--active"]/text()')
    )
    results = []
    for result in root.xpath('//p[@class="article"]'):
        pushlish_info = extract_first(
            result.xpath('.//*[@class="source"]/text()')
        ).split(",")
        publisher = pushlish_info.pop(0).strip() if len(pushlish_info) else ""
        publish_date = pushlish_info.pop(0).strip() if len(pushlish_info) else ""
        results.append(
            {
                "url": extract_first(result.xpath("./a/@href")),
                "title": extract_first(result.xpath('./a/*[@class="title"]/text()')),
                "preview_text": extract_first(
                    result.xpath('./span[@class="description"]/text()')
                ),
                "publisher": publisher,
...
def get_search_url(query: str, latest: bool = True, country: str = "us") -> str:
    return f"https://www.dogpile.com/serp?qc=news&q={quote(query)}"
