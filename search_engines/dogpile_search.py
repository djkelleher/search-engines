...
log_save_path = Path(__file__).parent.joinpath('logs/dogpile_search.log')
logger = get_logger("Dogpile Search", log_save_path)
...
async def parse_page(resp: ClientResponse,
    ...
    results = [{
        ...
        'title':
        extract_first(result.xpath('.//a[@class="web-bing__title"]/text()')),
        'preview_text':
        extract_first(
...
async def do_search(client: ClientSession,
                    ...
                    max_pages: int = 1000):
    start_url = 'https://www.dogpile.com/serp?qc=web&q=' + encode_url_str(
        query)
